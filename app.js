...
// GET all vehicles (model_id becomes model_name and hour_price) Price with PVM
app.get('/vehicles', async (req, res) => {
  ...
  vehicles.forEach((vehiclesItem) => {
    const result = model.filter((modelItem) => {
      return modelItem._id.toString() === vehiclesItem.model_id.toString();
    });

    for (let x of result) {
      const newObject = {
        ...vehiclesItem.toObject(),
        model_id: x.name,
